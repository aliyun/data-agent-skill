...
| `progress.log` | 执行日志，包含计划内容，阶段的结果等重要信息| **Agent 读取进度与结果** |
...
# 3. 检查 checkpoint 位点（判断是否有新进展）
tail -1 sessions/{session_id}/progress.log 
```

**状态说明：**
- `running` → 任务执行中，可读取 `progress.log` 获取阶段性结论
...
| 实时监控进展 | `tail -f sessions/{session_id}/progress.log \| jq '.data.content'` |
