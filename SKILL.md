...
| 文件 | 内容 | 用途 |
|------|------|------|
| `status.txt` | `running` / `waiting_input` / `completed` / `failed` | 快速判断任务状态（**注意：`waiting_input` 表示 Worker 已退出，需调用方主动操作**） |
| `progress.log` | 完整执行日志（含诊断信息、查询回显、步骤进度及最终结果） | **Agent 读取进度与结果的主要文件** |
| `checkpoint.txt`| 记录最后接收到的 `event.checkpoint` 数值 | 断点续传依据 |
| `result.json` | `{"status": "..."}` | 程序化状态检查 |
| `input.json` | 原始输入参数 | 复现/审计 |
| `worker.pid` | 当前 Worker 进程 PID | 并发防护（自动清理） |
...
# 2. 根据状态采取行动：
#    - running       → 继续轮询，同时可读取 progress.log 获取阶段性结论
#    - waiting_input → ⚠️ Worker 已退出！必须读取 progress.log 展示给用户，等用户确认后发送指令
#    - completed     → 读取 progress.log 获取完整结果
#    - failed        → 读取 result.json 获取错误信息

# 3. 读取结果
cat sessions/{session_id}/progress.log

# 4. 如果 status 为 waiting_input，读取 progress.log 展示给用户后，根据用户意愿发送确认
python3 data_agent_cli.py attach --session-id {session_id} -q "确认执行"

# 5. 会话结束后，如果有生成报告，可以一键下载
python3 data_agent_cli.py reports --session-id {session_id}
```
...
| 场景 | Agent 行为 |
|------|-----------|
| `status.txt` 为 `waiting_input` | **Worker 已退出，不会自动恢复！** 必须读取 `progress.log` 展示给用户，等用户确认后通过 `attach -q` 发送指令。**切勿继续轮询等待状态变化** |
| 读取到执行计划 (Plan) | **必须**暂停，展示给用户，等待用户确认 |
| 读取到 `ask_report_render`（将要为您绘制网页报告...） | **必须**暂停，展示给用户，等待用户确认 |
| 用户说"减少步骤"/"修改计划" | 使用 `attach` 发送新指令，**不要**创建新会话 |
| 用户说"确认" | 使用 `attach` 发送确认到对应会话 |
| 收到 "worker already running" 错误 | 同一 Session 已有任务在执行，等待完成后再操作 |
