...
| `identity.jwt.enabled` | `false` | Verify the upstream-signed identity token (Feishu Aily JWT). A token is used when present; without one the request falls back to the identity headers, so keep `auth_token` set. A token that is present but invalid is always rejected |
...
| `identity.session_name_claim` | `user_id` | Identity field used as the STS RoleSessionName segment: `user_id` \| `email` \| `enterprise_email` \| `employee_no` \| `tenant_id` \| `agent_id`. Applies to both the JWT and header paths; on the header path only `user_id`/`email` are available and any other choice falls back to the user id. Group matching always uses `user_id`/`email` regardless |
...
**`identity.jwt` — the identity Aily signs for you.** Feishu Aily can sign the end-user identity into a JWT (HS256) and send it as `x-aily-jwt` on every request, using a secret shown in its MCP editor. Because only the platform holds that secret, a verified token both identifies the user and proves the request came from the platform. Verification pins the algorithm to HS256 and requires `exp`. Claims consumed: `user_id`, `tenant_id`, `email`, `enterprise_email`, `employee_no`, `department_ids`, `agent_id`.\n\n**Token first, headers as fallback.** A token is used whenever the caller sends one, and it wins over any identity headers on the same request. A request with **no** token falls back to the header path, which keeps its own guards (`auth_token`, `require_identity`) — so **set `auth_token` whenever JWT is enabled**, otherwise a caller can simply omit the token to reach the forgeable headers. The server logs a warning at startup if that combination is left open. A token that is present but **invalid** is always rejected and is never retried against the headers, so a deliberately broken token cannot be used to downgrade.
...
1. **Identity intake** — a signed `x-aily-jwt` when the caller sends one (and `identity.jwt.enabled`), otherwise the plain user id / email headers (names configurable via `identity.headers`; defaults `x-aily-user` / `x-aily-email`). stdio transport has no headers, so identity mode applies to HTTP/SSE only.
2. **Legitimacy check** — a present token has its signature and `exp` verified (invalid → rejected outright); on the header path, if `auth_token` is set the token header must match. Either way this happens before any group resolution.
